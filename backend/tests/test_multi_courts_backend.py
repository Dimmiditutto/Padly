...
def ensure_second_court() -> tuple[str, str]:
    with SessionLocal() as db:
        ...
        if len(courts) == 1:
            second = Court(club_id=courts[0].club_id, name='Campo 2', sort_order=2, is_active=True)
            db.add(second)
            db.commit()
            db.refresh(second)
...
def test_admin_can_create_and_rename_court(client):
    ...
    created = client.post('/api/admin/courts', json={'name': 'Campo Centrale'})
    ...
    assert any(item['name'] == 'Campo Centrale' for item in listed.json()['items'])

    updated = client.put(f"/api/admin/courts/{created_payload['id']}", json={'name': 'Campo 2'})
