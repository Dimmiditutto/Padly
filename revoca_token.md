...
Usa `play_master.md` come contesto fisso, ma tratta il repository reale come ground truth quando il master e piu vecchio dello stato corrente del codice. Non rimettere in discussione la logica di business gia chiusa nelle fasi precedenti. Mantieni il codice coerente col repository reale.
...
## Stato reale del repository da assumere come vero

Questi fatti sono gia implementati nel codice attuale e non vanno reinventati:

- il backend `/play` esiste gia e copre create, join, leave, update, cancel e shared match lookup
- `Match` esiste gia e contiene `public_share_token_hash`
- il link condiviso oggi funziona tramite `GET /api/play/shared/{share_token}`
- `_ensure_match_share_token_hash(...)` allinea la colonna al token pubblico corrente
- il token pubblico match attuale e stabile e deterministico per `club_id + match_id`
- il frontend usa gia `/c/:clubSlug/play/matches/:shareToken`
- il frontend ha gia UI share/open in `PlayPage`, `MyMatches`, `MatchCard` e `SharedMatchPage`
- la copertura principale di regressione share/frontend oggi vive in `backend/tests/test_play_phase3.py` e `frontend/src/pages/PlayPage.test.tsx`

Questo prompt non deve introdurre una seconda architettura di share token: deve partire dallo stato reale sopra.
...
Oggi il token pubblico match e ancora stabile e derivato deterministicamente. Questo e sufficiente per la Fase 3, ma non consente controllo operativo sul singolo link:
...
Preferisci la soluzione meno invasiva coerente con il codice attuale.

Default obbligatorio:
- estendi `Match` invece di introdurre una tabella nuova, a meno che il repository reale dimostri un blocco tecnico concreto

Default funzionali obbligatori:
- il raw token condiviso nuovo deve essere opaco e randomico, non solo derivato deterministicamente da `club_id` e `match_id`
...
- la rotazione deve creare un nuovo token attivo e invalidare quello precedente nella stessa operazione logica
...
- i match gia esistenti che oggi usano il token deterministico devono continuare a funzionare in modo transitorio
- dopo la prima rotazione o revoca esplicita di uno di questi match, il vecchio link legacy non deve piu essere accettato
- la compatibilita legacy deve essere esplicita nel codice e documentata in un file stato dedicato
...
Parti dal modello `Match` reale ed estendilo con il minimo necessario per lifecycle share token.

Il modello finale deve consentire almeno:
- match proprietario
- `club_id`
- token hash attivo
- eventuale stato o timestamp che distingua token attivo da revocato
- timestamp creazione/emissione token attivo
- eventuale metadato minimo necessario a rigenerare il raw token senza salvarlo in chiaro
...
Chiudi almeno queste API private coerenti:
- `POST /api/play/matches/{match_id}/share-token/rotate`
- `POST /api/play/matches/{match_id}/share-token/revoke`

Le API devono restituire il match aggiornato nel contratto gia usato da `/play`, senza introdurre payload ridondanti o divergenti se non serve davvero.
...
- non introdurre eccezioni implicite per match annullati o non piu gestibili

### Frontend minimo

Aggiungi il minimo indispensabile nella UI `/play` per rendere la feature realmente usabile:
- azione per rigenerare link
- azione per disattivare link
- feedback chiaro ma sobrio all'utente
- gestione elegante del caso link revocato o non piu disponibile sulla shared page
...
Aggiungi o estendi test reali almeno per:
...
- compatibilita legacy per i match gia esistenti

Se tocchi il frontend, aggiungi o estendi test mirati per:
- rigenerazione link dal flusso `/play`
- disattivazione link dal flusso `/play`
- messaggio o stato corretto su shared page quando il link e revocato o non piu disponibile

## Verifica obbligatoria

Se tocchi il backend:
- usa `D:/Padly/PadelBooking/.venv/Scripts/python.exe`
- valida migration Alembic up/down se introduci colonne nuove
- esegui almeno i test mirati del modulo `/play`

Se tocchi il frontend:
- esegui i test mirati della/e pagina/e toccata/e
...
Se introduci una nuova migration, valida esplicitamente upgrade e downgrade. Se aggiungi un file test dedicato alla shared page, eseguilo esplicitamente oltre ai test sopra. Se scegli file test diversi, mantieni comunque verifiche mirate equivalenti.
